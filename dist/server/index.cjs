    ...
    async upsertUser(userData) {
      if (userData.email) {
        ...
        if (existing && existing.id !== userData.id) {
          ...
          const [user2] = await db.update(usersTable).set({ ...profileFields, updatedAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm3.eq)(usersTable.id, existing.id)).returning();
          return user2;
        }
      }
